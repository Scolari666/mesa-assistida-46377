-- ============================================================================
-- Adicionais: an optional catalog of paid extras (e.g. "Bacon", "Cebola
-- Caramelizada") that admins attach to individual products with a per-pair
-- max quantity, and customers pick per unit when adding a product to the
-- cart. Unlike variations (pick exactly one size/type), a customer can add
-- any number of different addons, each 0..max_quantity.
-- ============================================================================
...
CREATE TABLE public.product_addons (
  ...
  max_quantity INTEGER NOT NULL DEFAULT 4 CHECK (max_quantity > 0),
  sort_order INTEGER NOT NULL DEFAULT 0,
...
CREATE POLICY "Anyone can view product_addons"
...
-- Snapshot of the chosen addons for a cart line, same spirit as
-- variation_name: for display/history, never re-trusted after the fact.
ALTER TABLE public.order_items ADD COLUMN addons JSONB;

-- ---------------------------------------------------------------------------
-- compute_order_pricing: add addon validation/pricing into the items loop.
-- Each item's addons cost is folded into that item's unit_price (so
-- line_total = unit_price * quantity still holds), with a snapshot kept
...
DECLARE
  ...
  v_addon_max INT;
  v_addon_qty INT;
  v_addons_unit_cost NUMERIC(10,2);
  v_addons_out JSONB;
BEGIN
  ...
  LOOP
    ...
    v_addons_unit_cost := 0;
    v_addons_out := '[]'::jsonb;

    IF v_item -> 'addons' IS NOT NULL THEN
      FOR v_addon_json IN SELECT * FROM jsonb_array_elements(v_item -> 'addons')
      LOOP
        v_addon_qty := COALESCE((v_addon_json ->> 'quantity')::INT, 0);
        IF v_addon_qty <= 0 THEN
          CONTINUE;
        END IF;

        SELECT a.id, a.name, a.price, pa.max_quantity
          INTO v_addon_id, v_addon_name, v_addon_price, v_addon_max
        FROM public.addons a
        JOIN public.product_addons pa ON pa.addon_id = a.id
        WHERE a.id = (v_addon_json ->> 'addon_id')::UUID
          AND pa.product_id = v_product.id
          AND a.active = true;

        IF NOT FOUND THEN
          RAISE EXCEPTION 'INVALID_ADDON' USING ERRCODE = 'P0001';
        END IF;

        IF v_addon_qty > v_addon_max THEN
          RAISE EXCEPTION 'ADDON_QUANTITY_EXCEEDS_MAX' USING ERRCODE = 'P0001';
        END IF;

        v_addons_unit_cost := v_addons_unit_cost + (v_addon_price * v_addon_qty);
        v_addons_out := v_addons_out || jsonb_build_object(
          'addon_id', v_addon_id,
          'name', v_addon_name,
          'unit_price', v_addon_price,
          'quantity', v_addon_qty
        );
      END LOOP;
    END IF;

    v_unit_price := v_unit_price + v_addons_unit_cost;
...
-- ---------------------------------------------------------------------------
-- create_order: same as before, plus persisting each item's addons snapshot.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  INSERT INTO public.order_items (order_id, product_id, product_name, variation_id, variation_name, unit_price, quantity, line_total, notes, addons)
  SELECT
    v_order_id,
    (i ->> 'product_id')::UUID,
    i ->> 'product_name',
...
-- ---------------------------------------------------------------------------
-- get_order_by_stripe_session: include addons in the item snapshot too.
-- ---------------------------------------------------------------------------
